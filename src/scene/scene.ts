...
let changeBootMaterial = (
	...
	if (!paramaterValueItem) {
		throw new Error(
			'Could not find parameter value item for value ' + value
		)
	}
...
export function requireMesh(name: string): THREE.Mesh | undefined {
	let res : THREE.Mesh | undefined
	scene.traverse((child) =>{
		if(child.name === name) {
			res = child as THREE.Mesh
		}
	})
	return res
}